...
def get_cat_and_num_vars_lists(data_schema):      
    cat_vars, num_vars = [], []
    attributes = data_schema["inputDatasets"]["regressionBaseMainInput"]["predictorFields"]   
...
def get_cat_vars_with_missing_impute_for_na(data, pp_params, model_cfg): 
    threshold = model_cfg['max_perc_miss_for_most_freq_impute']
...
def get_preprocess_params(data, data_schema, model_cfg): 
    ...
    pp_params["id_field"] = data_schema["inputDatasets"]["regressionBaseMainInput"]["idField"]   
    
    # set the target attribute
    pp_params["target_attr_name"] = data_schema["inputDatasets"]["regressionBaseMainInput"]["targetField"]   
